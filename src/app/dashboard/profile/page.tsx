...
function Toast({ msg, type, onClose }: { msg: string; type: "success" | "error"; onClose: () => void }) {
  ...
  return (
    <div role="alert" style={{ position: "fixed", bottom: 28, right: 28, zIndex: 9999, padding: "12px 18px", borderRadius: 14, backdropFilter: "blur(20px)", background: c.bg, border: `1px solid ${c.bdr}`, color: c.text, fontFamily: "DM Sans, sans-serif", fontWeight: 500, fontSize: 13.5, display: "flex", alignItems: "center", gap: 10, maxWidth: 360, boxShadow: `0 8px 32px ${c.bdr}` }}>
      <span style={{ width: 20, height: 20, borderRadius: "50%", border: `1.5px solid ${c.text}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{type === "success" ? <IcCheck s={10} /> : "✕"}</span>
...
function UploadZone({ previewUrl, onFile, onError, loading, shape, height, label }: {
  ...
  const id = useId();
  const radius = shape === "circle" ? 9999 : 18;
  ...
  return (
    <div role="button" tabIndex={0} onClick={() => inputRef.current?.click()}
      ...
      {previewUrl ? (
        ...
        <div style={{ width: "100%", height: "100%", borderRadius: radius, background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)", border: `2px dashed ${isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 6 }}>
          {shape === "circle" ? <IcUser s={26} /> : <IcImage s={28} />}
          <span style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "DM Sans, sans-serif" }}>{label}</span>
        </div>
      )}
      <div className="prof-upload-overlay" style={{ position: "absolute", inset: 0, borderRadius: radius, background: "rgba(0,0,0,0.52)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5, opacity: loading ? 1 : undefined, pointerEvents: "none" }}>
        {loading ? <div className="prof-spin" /> : (<><IcCamera s={20} /><span style={{ fontSize: 10, color: "#fff", fontFamily: "DM Sans, sans-serif", fontWeight: 600 }}>Change</span></>)}
      </div>
      {shape === "circle" && (
...
function SectionCard({ title, icon, children, isDark }: { title: string; icon: React.ReactNode; children: React.ReactNode; isDark: boolean }) {
  return (
    <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 16, background: isDark ? "rgba(255,255,255,0.03)" : "#fff", border: `1px solid ${isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)"}`, boxShadow: isDark ? "none" : "0 2px 14px rgba(0,0,0,0.04)" }}>
...
function Field({ label, value, onChange, placeholder, type = "text", maxLength, hint, disabled, rows, error }: {
  ...
  return (
    <div>
      <label htmlFor={fid} style={{ display: "block", fontFamily: "DM Sans, sans-serif", fontWeight: 600, fontSize: 11.5, color: "var(--text-muted)", marginBottom: 6, letterSpacing: "0.06em", textTransform: "uppercase" }}>{label}</label>
      {rows
        ? <textarea id={fid} value={value} rows={rows} maxLength={maxLength} disabled={disabled} placeholder={placeholder} onChange={e => onChange?.(e.target.value)} style={inp} />
        : <input id={fid} type={type} value={value} maxLength={maxLength} disabled={disabled} placeholder={placeholder} onChange={e => onChange?.(e.target.value)} style={inp} />
      }
      {hasErr && <p role="alert" style={{ fontSize: 11.5, color: "#ef4444", fontFamily: "DM Sans, sans-serif", margin: "4px 0 0" }}>✕ {error}</p>}
      {hint && !hasErr && <p style={{ fontSize: 11.5, color: "var(--text-muted)", fontFamily: "DM Sans, sans-serif", margin: "4px 0 0" }}>{hint}</p>}
      {maxLength != null && value.length > 0 && (
        <p style={{ fontSize: 11, fontFamily: "DM Sans, sans-serif", margin: "3px 0 0", textAlign: "right", color: value.length > maxLength * 0.9 ? "#f59e0b" : "var(--text-muted)" }}>{value.length}/{maxLength}</p>
      )}
    </div>
  );
}
...
function CompletionRing({ pct, color }: { pct: number; color: string }) {
  const { isDark } = useTheme();
  const size = 54; const r = (size - 5) / 2; const c = 2 * Math.PI * r;
  const track = isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)";
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" strokeWidth={4} stroke={track} />
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" strokeWidth={4} stroke={color} strokeDasharray={c} strokeDashoffset={c * (1 - pct / 100)} strokeLinecap="round" style={{ transition: "stroke-dashoffset 0.8s ease" }} />
      </svg>
...
function ProfileSkeleton() {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 0 60px" }}>
      <div style={{ height: 180, background: b, borderRadius: 18, marginBottom: 60, animation: "pfpulse 2s ease-in-out infinite" }} />
      <div style={{ padding: "0 24px" }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ height: 200, background: b, borderRadius: 20, marginBottom: 16, animation: "pfpulse 2s ease-in-out infinite", animationDelay: `${i * 0.15}s` }} />
        ))}
      </div>
      <style>{`@keyframes pfpulse{0%,100%{opacity:.4}50%{opacity:.9}}`}</style>
    </div>
  );
}
    ...
    >
      <div style={{ position: "relative", marginBottom: 64 }}>
        <div style={{ position: "relative" }}>
          ...
          {bannerLoading && (
            <div style={{ position: "absolute", top: 8, right: 8, padding: "4px 10px", borderRadius: 8, background: "rgba(0,0,0,0.55)", color: "#fff", fontSize: 11, fontFamily: "DM Sans, sans-serif", backdropFilter: "blur(8px)" }}>
              Uploading…
            </div>
          )}
          {bannerPreview && !bannerLoading && bannerPreview !== (user?.bannerImageUrl ?? "") && (
            <div style={{ position: "absolute", top: 8, right: 8, padding: "4px 10px", borderRadius: 8, background: "rgba(255,107,0,0.85)", color: "#fff", fontSize: 11, fontFamily: "DM Sans, sans-serif", fontWeight: 600, backdropFilter: "blur(8px)" }}>
              PREVIEW
            </div>
          )}
        </div>
        <div style={{ position: "absolute", bottom: -44, left: 28, display: "flex", alignItems: "flex-end", gap: 14 }}>
          <div style={{ borderRadius: "50%", border: "3px solid var(--bg)", boxShadow: "0 0 0 2px rgba(255,107,0,0.4)", position: "relative" }}>
            <UploadZone previewUrl={avatarPreview} onFile={handleAvatar} onError={handleErr} loading={avatarLoading} shape="circle" height={80} label="Photo" />
            {avatarPreview && !avatarLoading && avatarPreview !== (user?.profileImageUrl ?? "") && (
              <div style={{ position: "absolute", bottom: -18, left: "50%", transform: "translateX(-50%)", padding: "2px 8px", borderRadius: 6, background: "rgba(255,107,0,0.9)", color: "#fff", fontSize: 9, fontFamily: "DM Sans, sans-serif", fontWeight: 700, whiteSpace: "nowrap" }}>
                PREVIEW
              </div>
            )}
          </div>
        ...
        <div style={{ position: "absolute", bottom: -42, right: 24, display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ padding: "0 24px" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 22 }} role="list" aria-label="Profile completion">
          {COMPLETION_FIELDS.map(f => {
            const done = draft ? f.check(draft) : false;
            return (
              <span key={f.label} role="listitem" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 999, fontSize: 11, fontFamily: "DM Sans, sans-serif", background: done ? "rgba(52,211,153,0.08)" : "rgba(255,136,0,0.08)", border: `1px solid ${done ? "rgba(52,211,153,0.25)" : "rgba(255,136,0,0.2)"}`, color: done ? "#34d399" : "#ff8800" }}>
        ...
        <SectionCard title="Basic Info" icon={<IcUser s={13} />} isDark={isDark}>
          <div style={G2} className="pfgrid">
            ...
            <div>
              <label htmlFor={genderSelId} style={{ display: "block", fontFamily: "DM Sans, sans-serif", fontWeight: 600, fontSize: 11.5, color: "var(--text-muted)", marginBottom: 6, letterSpacing: "0.06em", textTransform: "uppercase" }}>Gender</label>
              <select id={genderSelId} value={gender} onChange={e => { setGender(e.target.value); setIsDirty(true); }}
                style={{ width: "100%", padding: "10px 13px", borderRadius: 11, boxSizing: "border-box" as const, fontFamily: "DM Sans, sans-serif", fontSize: 14, outline: "none", border: `1px solid ${isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}`, background: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.025)", color: "var(--text)", cursor: "pointer" }}>
        ...
        <SectionCard title="Social Links" icon={<IcLink s={13} />} isDark={isDark}>
          <div style={G2} className="pfgrid">
        ...
        <SectionCard title="Location" icon={<IcPin s={13} />} isDark={isDark}>
          <div style={G2} className="pfgrid">
            <div style={{ gridColumn: "1/-1" }}>
        ...
        <SectionCard title="Professional" icon={<IcBriefcase s={13} />} isDark={isDark}>
          <div style={G2} className="pfgrid">
        ...
        <SectionCard title="Interests" icon={<IcTarget s={13} />} isDark={isDark}>
            ...
            {CATS.map(cat => {
              ...
              return (
                <button key={cat} type="button" aria-pressed={sel}
                  ...
                  style={{ padding: "7px 14px", borderRadius: 999, fontFamily: "DM Sans, sans-serif", fontSize: 13, cursor: "pointer", transition: "all 0.15s", background: sel ? "linear-gradient(135deg,rgba(255,107,0,0.15),rgba(255,204,0,0.1))" : (isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)"), border: `1px solid ${sel ? "rgba(255,107,0,0.4)" : (isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.07)")}`, color: sel ? "#ff8800" : "var(--text)", fontWeight: sel ? 600 : 400, boxShadow: sel ? "0 0 12px rgba(255,107,0,0.18)" : "none" }}>
                  {sel && <IcCheck s={9} />}{sel ? " " : ""}{cat}
                </button>
              );
            })}
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 14, paddingTop: 4 }}>
          {isDirty && (
            <span style={{ fontSize: 12, color: "#f59e0b", fontFamily: "DM Sans, sans-serif", fontWeight: 500 }}>Unsaved changes</span>
          )}
          <button type="button" onClick={save} disabled={saving}
            style={{ padding: "12px 28px", borderRadius: 14, border: "none", background: "linear-gradient(135deg,#ff6b00,#ffcc00)", color: "#fff", fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 14, boxShadow: "0 0 24px rgba(255,100,0,0.35)", position: "relative", overflow: "hidden", transition: "opacity 0.18s", display: "flex", alignItems: "center", gap: 8, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
      ...
      <style>{`
        ...
        @keyframes pfpulse{0%,100%{opacity:.4}50%{opacity:.9}}
        @keyframes pfspin{to{transform:rotate(360deg)}}
        .prof-spin{width:22px;height:22px;border-radius:50%;border:2.5px solid rgba(255,255,255,0.4);border-top-color:#fff;animation:pfspin .7s linear infinite}
        .prof-spin-sm{width:14px;height:14px;border-radius:50%;border:2px solid rgba(255,255,255,0.5);border-top-color:#fff;animation:pfspin .7s linear infinite;position:relative;flex-shrink:0}
