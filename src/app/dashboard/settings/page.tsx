    ...
    >
      <span
        style={{
          ...
          animation:
            "stShimmer 2.4s ease-in-out infinite",
        }}
      />
      {loading && (
        <span
...
function KycFormStep({
  ...
  const uploadDoc = useCallback(
    async (file: File, type: "pan" | "af" | "ab") => {
      ...
      try {
        ...
        showToast(
          `${
            { pan: "PAN", af: "Aadhaar Front", ab: "Aadhaar Back" }[type]
          } uploaded!`,
          "success",
        );
      } catch (e: unknown) {
  ...
  const submit = useCallback(async () => {
    ...
    if (!/^\d{4}-\d{4}-\d{4}$/.test(aadhaar))
      e.aadhaar = "Format: XXXX-XXXX-XXXX";
    ...
    panUrl, afUrl, abUrl, pan, aadhaar,
    acHolder, acNumber, ifsc, bankName, branch,
    upiId, panPid, afPid, abPid, showToast, onSubmitted,
  ]);
    ...
    <div>
      ...
      >
        ...
        <SInput
          ...
          value={aadhaar}
          onChange={(v) => {
            let r = v.replace(/\D/g, "").slice(0, 12);
            r = r.replace(
              /(\d{4})(\d{4})(\d{0,4})/,
              (_, a, b, c) =>
                c
...
function BecomeCreatorWizard() {
  ...
  const [kycData, setKycData] =
    useState<KycStatusResponse | null>(null);
  const [step, setStep] =
    useState<WizardStep>("intro");
  ...
  useEffect(() => {
    if (isCreator || kycStatus === "APPROVED")
      setStep("approved");
    else if (kycStatus === "PENDING_APPROVAL")
      setStep("submitted");
    else if (kycStatus === "REJECTED")
      setStep("rejected");
    else if (kycStatus === "PENDING_SUBMISSION")
  ...
  const verifyOtp = useCallback(async () => {
    ...
    try {
      ...
      if (rt) {
        try { await authApi.refresh(rt); } catch {}
      }
      showToast("Email verified! Submit KYC docs.", "success");
      await refetch();
      setStep("otp-verified");
      setOtpError("");
    } catch (e: unknown) {
  ...
  const handleKycSubmitted = useCallback(
    async (data: KycStatusResponse) => {
      ...
      await refetch();
      showToast(
        "KYC submitted! Review takes 24–48 hours.",
        "success",
      );
      setStep("submitted");
    },
  ...
  if (step === "approved") return (
    <div style={{ textAlign: "center", padding: "10px 0" }}>
      <div style={{ width: 52, height: 52, borderRadius: 15, margin: "0 auto 14px", background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#34d399" }}>
        <IcZap s={22} />
      </div>
      <p style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 16, color: "#34d399", margin: "0 0 6px" }}>Verified Creator</p>
      ...
      {kycData && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, textAlign: "left" }}>
          {([["PAN", kycData.panNumber], ["Bank", kycData.bankName], ["Account", kycData.maskedBankAccount], ["IFSC", kycData.bankIfscCode], ["UPI", kycData.upiId]] as [string, string|undefined][]).filter(([, v]) => v).map(([k, v]) => (
            <div key={k} style={{ padding: "9px 12px", borderRadius: 10, background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)", border: `1px solid ${isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)"}` }}>
  ...
  if (step === "submitted") return (
    <div style={{ textAlign: "center", padding: "10px 0" }}>
      <div style={{ width: 52, height: 52, borderRadius: 15, margin: "0 auto 14px", background: "rgba(167,139,250,0.1)", border: "1px solid rgba(167,139,250,0.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#a78bfa" }}>
        <IcClock s={22} />
      </div>
      <p style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 16, color: "#a78bfa", margin: "0 0 6px" }}>Under Review</p>
  ...
  if (step === "intro") return (
    <div>
      ...
      <div style={{ padding: 16, borderRadius: 12, background: "linear-gradient(135deg,rgba(255,107,0,0.08),rgba(255,204,0,0.04))", border: "1px solid rgba(255,107,0,0.18)", marginBottom: 20, display: "flex", gap: 14, alignItems: "flex-start" }}>
        <div style={{ width: 40, height: 40, borderRadius: 11, background: "rgba(255,107,0,0.12)", border: "1px solid rgba(255,107,0,0.2)", display: "flex", alignItems: "center", justifyContent: "center", color: "#ff8800", flexShrink: 0 }}>
          <IcRocket s={18} />
        </div>
        <div>
      ...
      {toast && (
        <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 10, background: toast.type === "error" ? "rgba(239,68,68,0.07)" : "rgba(52,211,153,0.07)", border: `1px solid ${toast.type === "error" ? "rgba(239,68,68,0.2)" : "rgba(52,211,153,0.2)"}`, color: toast.type === "error" ? "#ef4444" : "#34d399", fontFamily: "DM Sans, sans-serif", fontSize: 13 }}>
          {toast.msg}
        </div>
      )}
    </div>
  ...
  if (step === "otp-verified") return (
    <div>
      ...
      <div style={{ padding: "12px 16px", borderRadius: 10, background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.2)", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ color: "#34d399" }}><IcCheck s={16} /></div>
        <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: 13, color: "#34d399", margin: 0 }}>Email verified! Submit your KYC documents.</p>
      </div>
...
function AccountInfo() {
  ...
  const rows: [string, string][] = [
    ...
    ["Account", fmt(user.accountStatus)],
    ["KYC", fmt(user.kycStatus)],
    [
      "Member since",
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            Suspends your account. Reactivate any
            time via support.
          </p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showConfirm && (
          ...
          >
            ...
            >
              Contact{" "}
              <strong>support@crowdspark.in</strong>{" "}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Manage verification, creator status, and
          account details.
        </p>
      </motion.div>
      ...
      <Section
        title={
          isCreator ? "Creator Status" : "Become a Creator"
        }
        icon={<IcRocket s={13} />}
        subtitle={
          isCreator
