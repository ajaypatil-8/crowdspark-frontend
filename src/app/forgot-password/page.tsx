  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      nodes.forEach(n => {
        ...
        ctx.fillStyle = `hsla(${n.hue},80%,62%,${isDark ? 0.38 : 0.22})`; ctx.fill();
      });
...
// ── Animated Email Icon ─────────────────────────────────────────────────────
function AnimatedEmailIcon({ isDark }: { isDark: boolean }) {
  return (
    <motion.div
    ...
    >
      ...
      <div style={{
        ...
        boxShadow: "0 0 32px rgba(255,107,0,0.18)",
        position: "relative", zIndex: 1,
      }}>
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#ff8800" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
// ── Animated Lock Icon ──────────────────────────────────────────────────────
function AnimatedLockIcon({ isDark }: { isDark: boolean }) {
  return (
    <motion.div
      initial={{ scale: 0, rotate: -20 }}
      animate={{ scale: 1, rotate: 0 }}
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.9, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
      ...
      <div style={{ position: "relative", width: 88, height: 88, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 28 }}>
        {[1, 2, 3].map(i => (
          <motion.div key={i} initial={{ scale: 0, opacity: 0.7 }} animate={{ scale: 2.8, opacity: 0 }}
            ...
            style={{ position: "absolute", width: 60, height: 60, borderRadius: "50%", border: "1.5px solid rgba(255,136,0,0.4)" }} />
        ))}
      ...
      <motion.div initial={{ scaleX: 0, originX: "center" }} animate={{ scaleX: 1 }} transition={{ delay: 0.4, duration: 0.45 }}
        style={{ width: 48, height: 3, borderRadius: 2, background: "linear-gradient(90deg,#ff5500,#ffcc00)", marginBottom: 16, boxShadow: "0 0 14px rgba(255,107,0,0.5)" }} />
      <motion.p initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.32 }}
        style={{ fontFamily: "DM Sans, sans-serif", fontSize: 14.5, color: isDark ? "rgba(255,255,255,0.48)" : "rgba(0,0,0,0.5)", margin: "0 0 8px", lineHeight: 1.65 }}>
        We sent a reset link to
      </motion.p>
      <motion.p initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.38 }}
        style={{ fontFamily: "DM Sans, sans-serif", fontSize: 15, color: isDark ? "#eeeef5" : "#1a1a2e", margin: "0 0 28px", fontWeight: 600 }}>
        {email}
      </motion.p>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        style={{ fontFamily: "DM Sans, sans-serif", fontSize: 13, color: isDark ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.4)", margin: "0 0 28px", lineHeight: 1.6 }}>
      ...
      <motion.div initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.45 }} style={{ width: "100%" }}>
        <Link href="/login"
          ...
          <span style={{ position: "absolute", inset: 0, background: "linear-gradient(105deg,transparent 28%,rgba(255,255,255,0.2) 50%,transparent 72%)", animation: "fpShimmer 2.2s ease-in-out infinite" }} />
  ...
  return (
    <div style={{ minHeight: "100vh", background: pageBg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
      ...
      <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", opacity: isDark ? 0.022 : 0.014 }}>
        <defs><pattern id="fpg" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse"><path d="M 50 0 L 0 0 0 50" fill="none" stroke={isDark ? "#ff8800" : "#000"} strokeWidth="0.7" /></pattern></defs>
          ...
          {done ? (
            ...
            <motion.div key="form" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.4 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.5 }} style={{ marginBottom: 28 }}>
                <AnimatedEmailIcon isDark={isDark} />
                ...
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: 14.5, color: mutedClr, margin: 0, lineHeight: 1.65 }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <motion.div initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.28, duration: 0.4 }}>
                  <ForgeInput label="Email address" type="email" value={email} onChange={setEmail} autoComplete="email" isDark={isDark} disabled={loading} />
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.4 }}>
                  <motion.button type="submit" disabled={!canSubmit}
                    ...
                    {loading ? (
                      <><span style={{ width: 16, height: 16, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", animation: "fpSpin .65s linear infinite", flexShrink: 0 }} /><span style={{ position: "relative" }}>Sending reset link…</span></>
                    ) : (
                      <span style={{ position: "relative", display: "flex", alignItems: "center", gap: 8 }}>
                        Send reset link
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13" /><path d="M22 2L15 22 11 13 2 9l20-7z" /></svg>
                      </span>
                    )}
                  </motion.button>
                </motion.div>
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
                style={{ fontFamily: "DM Sans, sans-serif", fontSize: 13.5, color: mutedClr, textAlign: "center", margin: "22px 0 0" }}>
