...
function ry3(v:[number,number,number],a:number):[number,number,number]{const[x,y,z]=v;return[x*Math.cos(a)+z*Math.sin(a),y,-x*Math.sin(a)+z*Math.cos(a)];}
function rx3(v:[number,number,number],a:number):[number,number,number]{const[x,y,z]=v;return[x,y*Math.cos(a)-z*Math.sin(a),y*Math.sin(a)+z*Math.cos(a)];}
function proj3(v:[number,number,number],cx:number,cy:number,sc:number):[number,number,number]{const[x,y,z]=v;const f=5/(5+z*0.4)*sc;return[x*f+cx,-y*f+cy,z];}
...
function SceneCanvas({ isDark }: { isDark: boolean }) {
  ...
  useEffect(() => {
    ...
    const tick=()=>{
      ...
      orbs.forEach(o=>{
        o.x+=o.vx; o.y+=o.vy;
        if(o.x<-0.15||o.x>1.15)o.vx*=-1; if(o.y<-0.15||o.y>1.15)o.vy*=-1;
        const gx=o.x*w,gy=o.y*h,gr=o.r*Math.min(w,h);
        const g=ctx.createRadialGradient(gx,gy,0,gx,gy,gr);
        g.addColorStop(0,`hsla(${o.hue},82%,${isDark?58:50}%,${o.a})`); g.addColorStop(1,"transparent");
      ...
      se1.forEach(([i,j])=>{
        ...
        ctx.strokeStyle=`hsla(${20+t*32},95%,60%,${0.11+t*0.56})`; ctx.lineWidth=0.5+t*1.5;
        ctx.shadowColor=`hsla(${20+t*32},90%,55%,${t*0.42})`; ctx.shadowBlur=4+t*8; ctx.stroke();
      });
      pv1.forEach(([x,y],i)=>{
      ...
      se2.forEach(([i,j])=>{
        ...
        ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2);
        ctx.strokeStyle=`hsla(${195+t*55},88%,58%,${0.09+t*0.48})`; ctx.lineWidth=0.4+t*1.1;
        ctx.shadowColor=`hsla(${195+t*55},85%,55%,${t*0.36})`; ctx.shadowBlur=3+t*6; ctx.stroke();
      });
      pv2.forEach(([x,y],i)=>{
      ...
      if(frame%3===0) sparks.push({
        ...
        life:0,maxLife:65+Math.random()*110,r:1.3+Math.random()*3.2,hue:8+Math.random()*40,
      });
      ctx.save();
      ...
      sl.addColorStop(0,"transparent"); sl.addColorStop(0.5,`rgba(255,145,0,${isDark?0.035:0.02})`); sl.addColorStop(1,"transparent");
...
function FloatPill({p,isDark}:{p:typeof FLOAT_PILLS[0];isDark:boolean}){
  return(
    <motion.div
      ...
      style={{position:"absolute",left:p.x,top:p.y,paddingTop:10,paddingBottom:10,paddingLeft:16,paddingRight:16,borderRadius:14,
        background:isDark?"rgba(10,8,18,0.82)":"rgba(255,255,255,0.88)",
        border:`1px solid ${isDark?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.07)"}`,
        backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",
        boxShadow:isDark?"0 8px 32px rgba(0,0,0,0.55),inset 0 1px 0 rgba(255,255,255,0.05)":"0 8px 32px rgba(0,0,0,0.1)",
        whiteSpace:"nowrap",pointerEvents:"none",zIndex:4,
      }}
    >
      <motion.div animate={{y:[0,-5,0]}} transition={{duration:3+p.delay*0.5,repeat:Infinity,ease:"easeInOut"}}>
  ...
  return(
    <div style={{position:"relative"}}>
      <label style={{
        ...
        fontSize:lifted?9.5:14.5,fontFamily:"Syne, sans-serif",
        fontWeight:lifted?800:400,letterSpacing:lifted?"0.14em":0,
        textTransform:lifted?"uppercase":"none",
        color:focused?"#ff8800":lifted?(isDark?"rgba(255,255,255,0.38)":"rgba(0,0,0,0.38)"):(isDark?"rgba(255,255,255,0.28)":"rgba(0,0,0,0.3)"),
      ...
      <input
        ...
        style={{
          ...
          paddingTop:lifted?24:16,paddingBottom:lifted?8:16,paddingLeft:16,paddingRight:suffix?50:16,
          borderRadius:14,outline:"none",fontFamily:"DM Sans, sans-serif",fontSize:14.5,color:"var(--text)",
          border:`1.5px solid ${focused?"rgba(255,107,0,0.7)":isDark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.1)"}`,
          background:focused?(isDark?"rgba(255,107,0,0.06)":"rgba(255,107,0,0.03)"):(isDark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.022)"),
          boxShadow:focused?"0 0 0 3.5px rgba(255,107,0,0.13),inset 0 1px 0 rgba(255,200,100,0.05)":"none",
          transition:"all 0.22s cubic-bezier(0.4,0,0.2,1)",
        }}
      />
      {focused&&(
...
function GithubIcon(){return(<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/></svg>);}

function MouseSpotlight({isDark}:{isDark:boolean}){
  const ref=useRef<HTMLDivElement>(null);
  useEffect(()=>{
    const el=ref.current; if(!el) return;
    const move=(e:MouseEvent)=>{
      el.style.background=`radial-gradient(600px circle at ${e.clientX}px ${e.clientY}px,rgba(255,107,0,${isDark?0.045:0.022}) 0%,transparent 60%)`;
    };
    window.addEventListener("mousemove",move);
    return ()=>window.removeEventListener("mousemove",move);
  },[isDark]);
  return <div ref={ref} style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:1,transition:"background 0.12s ease"}}/>;
}
...
const [pendingToken,    setPendingToken]    = useState("");
  ...
  return(
    <div style={{minHeight:"100vh",background:pageBg,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
      ...
      <MouseSpotlight isDark={isDark}/>

      <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none",opacity:isDark?0.028:0.016,zIndex:1}}>
        <defs><pattern id="lg" x="0" y="0" width="52" height="52" patternUnits="userSpaceOnUse">
          <path d="M 52 0 L 0 0 0 52" fill="none" stroke={isDark?"#ff8800":"#000"} strokeWidth="0.8"/>
        </pattern></defs>
        <rect width="100%" height="100%" fill="url(#lg)"/>
      </svg>

      <div style={{position:"absolute",top:0,left:0,right:0,height:200,background:`linear-gradient(to bottom,${isDark?"rgba(6,5,10,0.55)":"rgba(243,242,238,0.55)"} 0%,transparent 100%)`,pointerEvents:"none",zIndex:1}}/>
      <div style={{position:"absolute",bottom:0,left:0,right:0,height:200,background:`linear-gradient(to top,${isDark?"rgba(6,5,10,0.55)":"rgba(243,242,238,0.55)"} 0%,transparent 100%)`,pointerEvents:"none",zIndex:1}}/>
      ...
      {/* Theme Toggle */}
      <motion.div initial={{opacity:0,y:-22}} animate={{opacity:1,y:0}} transition={{duration:0.6,ease:"easeOut",delay:0.1}}
      ...
      {/* Card */}
      <motion.div
        initial={{opacity:0,y:38,scale:0.91}} animate={{opacity:1,y:0,scale:1}}
        transition={{duration:0.72,ease:[0.22,1,0.36,1]}}
        style={{
          position:"relative",zIndex:5,width:"100%",maxWidth:462,margin:"0 16px",
          background:cardBg,backdropFilter:"blur(30px) saturate(1.5)",WebkitBackdropFilter:"blur(30px) saturate(1.5)",
      ...
      >
        ...
        <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:0.18,duration:0.5}} style={{marginBottom:34}}>
          <h1 style={{fontFamily:"Syne, sans-serif",fontWeight:900,fontSize:"clamp(30px,4vw,42px)",color:"var(--text)",letterSpacing:"-0.03em",margin:"0 0 8px",lineHeight:1.06}}>
            Welcome back
          </h1>
          <motion.div initial={{scaleX:0,originX:"left"}} animate={{scaleX:1}} transition={{delay:0.45,duration:0.5,ease:"easeOut"}}
          ...
          <p style={{fontFamily:"DM Sans, sans-serif",fontSize:14.5,color:mutedClr,margin:0}}>Sign in to your CrowdSpark account</p>
        </motion.div>

        <AnimatePresence>
          {error&&(
            <motion.div initial={{opacity:0,y:-8,scale:0.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,scale:0.97}}
              style={{paddingTop:12,paddingBottom:12,paddingLeft:16,paddingRight:16,borderRadius:13,background:"rgba(239,68,68,0.07)",border:"1px solid rgba(239,68,68,0.22)",marginBottom:24,color:"#ef4444",fontSize:13.5,fontFamily:"DM Sans, sans-serif",display:"flex",alignItems:"flex-start",gap:10}}>
        ...
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <motion.div initial={{opacity:0,x:-14}} animate={{opacity:1,x:0}} transition={{delay:0.33,duration:0.4}}>
            <ForgeInput
              ...
              suffix={<button type="button" onClick={()=>setShowPass(v=>!v)} style={{background:"none",border:"none",cursor:"pointer",padding:0,color:"inherit",display:"flex",alignItems:"center"}}><EyeIcon off={showPass}/></button>}
            />
          </motion.div>
          ...
          <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.42,duration:0.4}}>
            <motion.button type="submit" disabled={!canSubmit}
              ...
              style={{
                ...
                background:canSubmit?"linear-gradient(135deg,#ff5500 0%,#ff8800 50%,#ffcc00 100%)":(isDark?"rgba(255,255,255,0.07)":"rgba(0,0,0,0.07)"),
                color:canSubmit?"#fff":(isDark?"rgba(255,255,255,0.22)":"rgba(0,0,0,0.22)"),
                fontFamily:"Syne, sans-serif",fontWeight:800,fontSize:15.5,
                cursor:canSubmit?"pointer":"not-allowed",position:"relative",overflow:"hidden",
              ...
              {loading?(
                <><span style={{width:16,height:16,borderRadius:"50%",border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",animation:"lpSpin .65s linear infinite",flexShrink:0}}/><span style={{position:"relative"}}>Signing in…</span></>
              ):(
        ...
        <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.55,duration:0.4}}
          ...
          {[
            ...
            <motion.a key={label} href={href}
              whileHover={{y:-2,borderColor:"rgba(255,136,0,0.42)",boxShadow:"0 8px 28px rgba(0,0,0,0.16)"}} whileTap={{scale:0.97}}
